...
function SSBSets(target: string) {
	...
	buf += `<h1>${Chat.escapeHTML(name)}</h1>`;
	...
	if (!Array.isArray(set.item)) {
		if (!baseDex.getItem(set.item).exists) {
	...
	if (!Array.isArray(set.ability)) {
		if (!baseDex.getAbility(set.ability).exists) {
...
export const commands: ChatCommands = {
	...
	ssbhelp: [
		`/ssb [staff member] - Displays a staff member's SSB set and custom features.`,
	],
};
